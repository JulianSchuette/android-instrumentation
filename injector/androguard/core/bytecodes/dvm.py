...
class EncodedMethod:
    ...
    def get_information(self):
        ...
        if self.code:
            ...
            nb = self.code.registers_size
            proto = self.get_descriptor()
            ret = proto.split(')')
            params = ret[0][1:].split()
            inner_offset = 0
            for p in params:
                if get_type(p) in ['long', 'double']:
                    inner_offset += 1
            if params:
...
#        print i, hex(h[ i ])
    ...
    def get_field_for_class(self, cl, name) :
        """
            Return a list all fields which corresponds to the regexp
        ...
        for i in self.classes.class_def :
            if i.get_name() == cl:
                for j in i.get_fields() :
                    if prog.match( j.get_name() ) or name == j.get_name() :
                        l.append( j )
        return l
    
    def get_exact_field_for_class(self, cl, name) :
        """
            Return a list all fields which corresponds to the regexp
